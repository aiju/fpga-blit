...
uartrproc(void *)
{
	...
	for(;;){
		recv(uartrxch, &c);
		reg_poll(UART_IN_STATUS, 1, 0);
		reg_write(UART_IN_DATA, c);
	}
}
...
telnetinit(char *dialstr)
{
	...
	proccreate(uartrproc, nil, mainstacksize);
	proccreate(uartwproc, nil, mainstacksize);
}
